...
fn main() {
    tracing_subscriber::registry()
        .with(tracing_timetree::layer().with_min(Duration::from_millis(1)))
        .init();
    ...
    {
        let _s = info_span!("parse").entered();
        sleep(Duration::from_millis(5));
        {
            let _s = info_span!("tokenize").entered();
            sleep(Duration::from_millis(8));
        }
        {
            let _s = info_span!("typecheck").entered();
...

    {
        ...
        for i in 0u32..500 {
            let _s = info_span!("iter", i).entered();
